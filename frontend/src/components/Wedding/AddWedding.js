...
export default function AddWedding({ setShowAdd, token, setWeddings, weddings }) {
  ...
  const formSubmit = (e) => {
    ...
    API.createWedding(formState, token)
      .then((res) => {
        setWeddings([...weddings, res.data.weddingData]);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    ...
    <form className="mt-3 bg-slate-200/90 p-2 rounded-lg shadow-xl z-10" id="addWedding" onSubmit={formSubmit}>
